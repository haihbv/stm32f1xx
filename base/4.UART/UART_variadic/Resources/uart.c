...
volatile uint8_t UARTx_dataReceived[3][100] = {"0\0", "0\0", "0\0"};
...
uint8_t UARTx_ReceiveData(UARTx_e Ux, uint8_t *data)
{
	...
	uint8_t index = 0;
	if(UARTx_rx_flag[Ux]) {
		for(volatile int i = 0; i< 0x1ffff; i++); // delay 1 lúc
...
{
	if(UARTx_rx_flag[U1] == 0){
		UARTx_rx_flag[U1] = 1;
	}
	UARTx_dataReceived[U1][UARTx_idx[U1]++] = USART1->DR;
}
void USART2_IRQHandler(void)
{
	if(UARTx_rx_flag[U2] == 0){
		UARTx_rx_flag[U2] = 1;
	}
	UARTx_dataReceived[U2][UARTx_idx[U2]++] = USART2->DR;
}
void USART3_IRQHandler(void)
{
	if(UARTx_rx_flag[U3] == 0){
		UARTx_rx_flag[U3] = 1;
	}
	UARTx_dataReceived[U3][UARTx_idx[U3]++] = USART3->DR;
}